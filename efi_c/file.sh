...
sudo cp /media/flash/one/EFI/BOOT/FILE.TXT /media/flash/two/EFI/BOOT/
...
sudo dd if=/dev/mapper/loop30p2 of="$DISK1"
sudo dd if=/dev/mapper/loop30p2 of="$DISK2"
sudo dd if=/dev/mapper/loop30p2 of="$DISK3"
